...
* 注意
  * この実行環境は他の人と共有されていません. 変更内容や, 実行結果が他の人と共有されることもないので自分用の作業環境として使えます
  * <font color="red">無操作で数時間放置すると自動的になくなります</font>
  * したがって, 行った設定作業 (以下の指示に基づく作業) を行っても数時間放置すると失われてしまいますのでご注意下さい
  * 得られた出力 (認定結果.xlsx) も同様ですので, ダウンロードする必要があるでしょう
...
* 以下はこちらで用意したダミーデータ. 誰でも読めるものになっているので試しにそのまま実行してみてもOK
...
* **<font color=red>データについて (重要)</font>**
...
print(f"学生一覧は {PROGRAM_STUDENTS.shape[0]} 行あります. 最初の20行は以下です")
PROGRAM_STUDENTS.head(20)
""" """
...
print(f"UTASデータは {UTAS_GRADE.shape[0]} 行あります. 最初の20行は以下です")
UTAS_GRADE.head(20)
""" """
...
print(f"科目一覧は {PROGRAM_COURSES.shape[0]} 行あります. 最初の20行は以下です")
PROGRAM_COURSES.head(20)
""" """
